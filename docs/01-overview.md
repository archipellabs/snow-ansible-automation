...
The controller is **AAP or AWX**. The same integration, governance, and content run on either, so the
...
- **Pull** — EDA *polls* ServiceNow's incident table (every 10 s); a matching incident launches the
  remediation job. Nothing inbound is exposed, ServiceNow needn't even know the controller exists, and it
  self-heals by re-polling.
...
It doesn't just automate — it automates **without bypassing control**. The governance an enterprise
expects is built in, not bolted on:
...
The automation acts on a real estate, not throwaway hosts: **Meridian Group**, a fictional but coherent
enterprise — support teams, business services, apps, servers, a populated CMDB, business users, an
identity provider. One manifest (`simulator/fleet.yml`) defines it all and materialises it everywhere:
containers, the ServiceNow CMDB, Keycloak, the controller's inventory. So a ServiceNow incident maps
cleanly to a real server, its team, and the right playbook.
...
demonstrated end-to-end, but **not production-hardened**. The honest limits and the hard-won lessons are
in **[11 · Notes](11-notes.md)**.
