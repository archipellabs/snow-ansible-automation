...
The bootstrap controller config (bootstrap/6A_aap/controller/configure.py) keeps its own create-heavy
...
class Aap:
    ...
    def jt_id(self, name):
        ...
        if not res:
            sys.exit(f"job template {name!r} not found — run bootstrap/6A_aap/controller/configure.py")
        return res[0]["id"]
