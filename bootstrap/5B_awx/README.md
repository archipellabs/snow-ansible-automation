...
| **B** | **Decision Environment** | easy | build `snow-eda-de` with `ansible-builder` → push to the in-cluster `registry:2` |
...
| **D** | **Controller config-as-code** | easy | ✅ **Done** — `controller/configure.py` (the `awx` twin; AAP-only "AAP Config" cred + "Configure EDA" JT dropped). Live: project synced, **CMDB inventory → 9 hosts**, 13 job templates with Target SSH + ServiceNow creds. *Job→fleet connectivity (k3s pods vs `host.containers.internal`) is a chantier E/H concern, not config.* |
| **E** | **EDA config (activations)** | easy-med | reuse `5A_aap/eda/*` against eda-server (same EDA API) — controller-cred host → AWX `/api/v2/`; event-stream URLs via ingress |
