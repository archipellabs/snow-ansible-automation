#!/usr/bin/env bash
# Laptop-side: push bootstrap/5B_awx/ + the repo-root .env to the AWX VM and run install.sh there
...
# session mid-run, so we detach it and reconnect to follow ~/5B_awx/install.log until it finishes.
#   AWX_FQDN=archipellabs-awx.<region>.cloudapp.azure.com ./bootstrap/5B_awx/sync.sh
set -euo pipefail
cd "$(dirname "$0")"
ENVFILE="../../.env"
...
rsync -avz -e "$SSH" --exclude '__pycache__' --exclude '.env' ./ "azureuser@${FQDN}:~/5B_awx/"
[ -f "$ENVFILE" ] && rsync -avz -e "$SSH" "$ENVFILE" "azureuser@${FQDN}:~/5B_awx/.env"

echo ">> starting install.sh (detached) on ${FQDN}…"
$SSH "azureuser@${FQDN}" "cd ~/5B_awx && FQDN='${FQDN}' setsid bash install.sh >install.log 2>&1 </dev/null & echo \$! >install.pid; echo 'install pid '\$(cat install.pid)"

echo ">> following install.log (reconnects through the k3s network reset; Ctrl-C is safe)…"
for a in 1 2 3 4 5 6; do
  $SSH "azureuser@${FQDN}" '
    cd ~/5B_awx 2>/dev/null || exit 1
